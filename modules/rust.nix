# The native packages: the server, the desktop client, and the same client
# compiled to wasm with the shell that loads it.
{
  perSystem = { pkgs, toolchain, rustPlatform, icedLibs, sources, ... }:
    let
    ...
    {
      packages = {
        inherit harken-server harken-iced harken-web;
