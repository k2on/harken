...
# `scripts/mutators.sh` as derivations, built by the engine's own library from
# the engine `flake.lock` pins.
#
# The checks need it and not merely the tests do: `foreign_peer!` does
# `include_bytes!` of the module, so *compiling* the crate with
# `--all-features` needs the file to exist. That is why `just lint` depends
